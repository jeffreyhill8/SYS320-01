...
while($operation){
    ...
    elseif($choice -eq 3){ 
        ...
        if ((checkPassword $Password) -eq $false) {
            Write-Host "Password does not meet minimum requirements, Please Try Again,"
            return
        }
    ...
    elseif($choice -eq 8){
        ...
        $failedLogins = Read-Host "Enter number of failed logins"
        $userLogins = getFailedLogins $failedLogins
        # TODO: Change the above line in a way that, the days 90 should be taken from the user
    ...
    elseif($choice -eq 9){
        ...
        $atRisk = $userLogins | Group-Object -Property User | Where-Object { $_.Count -gt 10 } | Select-Object Name, Count
